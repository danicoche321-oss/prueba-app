...
# Replace ExerciseCard header background colors and styles
# 1. Update the Sets Table container background from bg-stone-50/30 to bg-[var(--surface-1)] or transparent
content = content.replace('bg-stone-50/30 border-t border-stone-100', 'bg-[var(--surface-1)] border-t border-[var(--border-subtle)]')

# 2. Update the SetRow layout
                                    ...
                                    .map(({ set, index }) => {
                                        ...
                                        return (
                                            <div key={set.id} className={`flex items-center gap-2 py-1 ${set.isCompleted ? 'opacity-60' : ''}`}>
                                                <div className="w-8 flex-shrink-0 text-center text-[10px] font-bold text-[var(--text-3)] bg-[var(--surface-2)] rounded-md py-1.5">
                                                    {index + 1}
                                                </div>
...
with open('index.html', 'w', encoding='utf-8') as f:
    f.write(content)
print('Replaced sets table successfully')
